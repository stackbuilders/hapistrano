...
ensureCacheInPlace repo deployPath maybeRelease = do
  ...
  ensureRepositoryIsUpdated repo cpath
  unless exists $
...
-- | Update the repository origin on every deployment.
ensureRepositoryIsUpdated
  :: String            -- ^ Repo URL
  -> Path Abs Dir      -- ^ Repo path
  -> Hapistrano ()
ensureRepositoryIsUpdated = undefined
